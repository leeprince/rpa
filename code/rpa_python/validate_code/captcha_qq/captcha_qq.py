...
class Login(object):
    ...
    def login_main(self):
        ...
        click_keyi_username = driver.find_element(by=By.XPATH, value="//div[@class='wp-onb-tit']/a[text()='可疑用户']")
        self.webdriverwait_click(driver, click_keyi_username)

        login_button = driver.find_element(by=By.ID, value='code')
        self.webdriverwait_click(driver, login_button)
        time.sleep(1)

        driver.switch_to.frame(driver.find_element(by=By.ID, value='tcaptcha_iframe'))  # switch 到 滑块frame
        time.sleep(0.5)
        bk_block = driver.find_element(by=By.XPATH, value='//img[@id="slideBg"]')  # 大图
        # 渲染大小
        web_image_size = bk_block.size
        # 渲染大小的宽
        web_image_width = web_image_size['width']
        # 元素在可渲染画布中的位置
        bk_block_x = bk_block.location['x']

        slide_block = driver.find_element(by=By.XPATH, value='//img[@id="slideBlock"]')  # 小滑块
        # 元素在可渲染画布中的位置
        slide_block_x = slide_block.location['x']

        #  prince.lee@commt 2022/5/31 17:49
        print("获取 遮罩图/裁剪图 元素的尺寸>>>>>\r\n"
              "\t web_image_size：%s \r\n"
              "\t web_image_width：%s \r\n"
              "\t bk_block_x：%s \r\n"
              "\t slide_block_x：%s \r\n"
              % (web_image_size, web_image_width, bk_block_x, slide_block_x))

        bk_block = driver.find_element(by=By.XPATH, value='//img[@id="slideBg"]').get_attribute('src')  # 大图 url
        slide_block = driver.find_element(by=By.XPATH, value='//img[@id="slideBlock"]').get_attribute(
            'src')  # 小滑块 图片url
        slid_ing = driver.find_element(by=By.XPATH, value='//div[@id="tcaptcha_drag_thumb"]')  # 滑块
        ...
        # 真实滑块背景图片中的｀缺块｀所在的位置。返回的 (x, y) 对应的 (y, x) 元组，即 y=position[0] x=position[1]
        position = self.get_postion('./image/bkBlock.png', './image/slideBlock.png')
        # 渲染滑块背景图片出来后的｀缺块｀所在的位置的x方向偏移量
        real_scale_position = position[1] / width_scale
        # 遮罩图中存在裁剪图的所在的位置。返回的 (x, y) 对应的 (y, x) 元组，即 y=position[0] x=position[1]
        real_position = real_scale_position - (slide_block_x - bk_block_x)
        track_list = self.get_track(real_position + 4)

        #  prince.lee@commt 2022/5/31 17:52
        print("通过已保存的本地遮罩图/裁剪图，获取位置信息 >>>>>\r\n"
              "\t real_width：%s \r\n"
              "\t width_scale：%s \r\n"
              "\t position：%s \r\n"
              "\t real_scale_position：%s \r\n"
              "\t real_position：%s \r\n"
              "\t track_list：%s \r\n"
              % (real_width, width_scale, position, real_scale_position, real_position, track_list))
